<template>
  ...
  <div v-for="(controller,idx) in controllerArray">
    <!--    <router-link
            :to="`/admin/${controller.parameter}`">
          &lt;!&ndash;          @click="isClicked[idx] =! isClicked[idx]"&ndash;&gt;
          &lt;!&ndash;      @click="getData(controller.getter())"&ndash;&gt;
          <div @click="isClicked[idx] =! isClicked[idx]"
          >{{ controller.name }}
          </div>
        </router-link>
        <router-view :title="controller.title"/>-->
    <!--    <div>
    ...
    <div v-for="(value,key,idx) of useStore()">
      ...
      <div class=" " v-if="isExpanded[idx]">
        <template v-if="key === controller.parameter">
          <div class="mb-4 rounded-xl mx-auto border border-black max-w-fit p-2" v-for="k of value">
            <div class="grid grid-cols-2 justify-items-center mx-auto" v-for="(frst,sec) of k">
              ...
              <div class="grid grid-cols-2 mt-2   " v-if="frst !== null">
...
<!--                           @click="patching(controller.parameter,k.id, sec, )">-->
      ...
      <!--      getData(value[0].id)-->
      <!--      <input v-if="key==='getter'" type="button" value="getter" class="bg-blue-400"
...
const query = () =>{
  let string = ''
  for (let item in fields.value){
    console.log(fields.value[item]||null)

  }
}
const patching = async (path, id, key, value, isAdmin = false) => {
  try {
    const response = await patchAdmin({
      url: (isAdmin?'admin/':null) + path + '/' + id+'?' + key + '=' + value,
    })
...
const controllerArray = [
  {
    ...
    getter: async () => {
      try {
        const response = await instance({url: 'playlists'})
